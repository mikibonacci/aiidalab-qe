...
def export_xps_data(node):
    chemical_shifts = {}
    symmetry_analysis_data = node.outputs.symmetry_analysis_data.get_dict()
    equivalent_sites_data = symmetry_analysis_data["equivalent_sites_data"]
    if "chemical_shifts" in node.outputs:
        for key, data in node.outputs.chemical_shifts.items():
            ele = key[:-4]
            chemical_shifts[ele] = data.get_dict()
    binding_energies = {}
    if "binding_energies" in node.outputs:
        for key, data in node.outputs.binding_energies.items():
            ele = key[:-3]
            binding_energies[ele] = data.get_dict()
    spectra_cls = {}
    if "xps_spectra_cls" in node.outputs:
        for key, data in node.outputs.xps_spectra_cls.items():
            ele = key[:-12]
            X = data.get_x()[1]
            Y = data.get_y()
            array_y = []
    ...
    spectra_be = {}
    if "xps_spectra_be" in node.outputs:
        for key, data in node.outputs.xps_spectra_be.items():
            ele = key[:-11]
            X = data.get_x()[1]
            Y = data.get_y()
            array_y = []
